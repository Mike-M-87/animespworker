...
<details class="hover:bg-black/20 border shadow-[5px_0_5px_5px] shadow-white/10 border-white/40 rounded-xl">
    ...
    <form method="post" class="flex flex-col gap-4 px-3 pb-3">
      ...
      <div
        ...
        <div class="w-full flex sm:flex-row flex-col sm:items-center gap-2 py-1">
          <input type="url" name="sourcelink" value="${anime?.customSourceLink}"
            ...
            class="bg-transparent border rounded-lg p-1 flex-grow text-sm outline-none border-white/20">

          <div class="flex items-center justify-between gap-2">
            <select name="customDay" required
              class="p-1 border border-white/20 bg-transparent rounded-lg text-sm placeholder:text-muted-foreground/90 focus:outline-none focus:ring-0 sm:leading-7 text-foreground">
            ...
            <input type="time" required name="customTime" value="${anime?.customTime}"
              class="bg-transparent text-sm border border-white/20 rounded-lg">
          </div>
        </div>
      </div>
      `: ''}
      ...
      <div
        ...
        <label class="text-xs font-medium text-muted-foreground group-focus-within:text-white text-gray-400">Season
        </label>
      ...
      <div
        ...
        <div class="flex items-center">
          <input type="url" name="image_url" placeholder="https://..." autocomplete="off" value="${anime.image_url}"
            class="block w-full border-0 bg-transparent py-1 text-sm  placeholder:text-muted-foreground/90 focus:outline-none focus:ring-0 sm:leading-7 text-foreground">
        </div>
      </div>
